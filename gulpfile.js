...
function bundle(dev, moduleArr) {
  ...
  var entries = [helpers.getBuiltPrebidCoreFile(dev)].concat(
...
gulp.task('test-coverage', ['clean'], function(done) {
  new KarmaServer(
    karmaConfMaker(true, false, false, argv.file),
    newKarmaCallback(done)
  ).start();
});
...
gulp.task('e2etest', ['devpack', 'webpack'], function() {
  ...
  if (argv.browserstack) {
    ...
    cmdStr =
      cmdStr +
      ' --reporter ./test/spec/e2e/custom-reporter/pbjs-html-reporter.js';
    ...
    Object.keys(browsers)
      .filter(function(v) {
        return (
          v.substring(0, startWith.length) === startWith &&
          browsers[v].browser !== 'iphone'
        );
      })
